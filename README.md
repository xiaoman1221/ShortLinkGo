...
- **系统管理** - 仅管理员/超级管理员可访问；用户管理（超级管理员可改角色、封禁）；网站基本信息（名称/简介/菜单栏 Logo）；SMTP 配置与测试邮件
...
- **访问留痕与 GeoIP** - 每次访问记录 IP、国家/城市（可选 GeoLite2）、UA、来源
...
| `TRUSTED_PROXIES` | 可信代理 IP/CIDR（逗号分隔）；留空不信任任何代理头，反代部署必配 | -       |
